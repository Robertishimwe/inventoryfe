...
const DeletePopup = ({ unit, setIsDeletePopupOpen }) => {
  ...
  const { mutate: deleteProduct, isError: isDeleteError } = useMutation({
    mutationFn: async () => {
      const response = await api.delete(`/api/units/${unit.id}/softDelete`);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded shadow-lg z-50 relative w-1/3">
        <h2 className="text-lg font-semibold mb-4 text-center">Delete Unit</h2>
        <form onSubmit={handleDeleteFormSubmit}>
          <div className="mb-4">
            <label htmlFor="unit" className="block mb-2 font-medium">
              <span className="text-sm font-medium flex justify-center">
                Are you sure you want to delete &nbsp;<b>{unit.unitName}</b>?
              </span>
            </label>
          </div>
