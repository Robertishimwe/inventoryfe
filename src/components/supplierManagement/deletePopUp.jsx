...
const DeletePopup = ({ supplier, setIsDeletePopupOpen }) => {
  ...
  const { mutate: deleteProduct, isError: isDeleteError } = useMutation({
    mutationFn: async () => {
      const response = await api.delete(`/api/supplier/${supplier.id}/softDelete`);
      return response.data;
    },
    onSuccess: () => {
      toast.success(`"${supplier?.supplierName}" category was deleted successfully`);
      // Refetch the supplier data to update the data grid
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded shadow-lg z-50 relative w-1/3">
        <h2 className="text-lg font-semibold mb-4 text-center">Delete Suppliers</h2>
